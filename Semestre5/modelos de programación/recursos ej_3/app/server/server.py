...
def manejar_cliente(conexion, direccion, id_cliente):
    try:
        ...
        while True:
            ...
            mensaje = datos.decode('utf-8')  # Cambiado a .decode
            if mensaje.lower() == 'terminar':
                resumen = f"Resumen final - Aciertos: {aciertos}, Desaciertos: {desaciertos}"
                conexion.send(resumen.encode('utf-8'))  # Cambiado a .encode
                break
            ...
            try:
                ...
                disponibles = list(map(int, conexion.recv(1024).decode('utf-8').split(',')))  # Cambiado a .decode
                ...
                for i in range(3):
                    ...
                    conexion.send(response.encode('utf-8'))  # Cambiado a .encode
                ...
                conexion.send(resumen_ronda.encode('utf-8'))  # Cambiado a .encode
                
                if desaciertos_ronda == 3:
                    break
                
            except ValueError:
                conexion.send("Error: Número no válido".encode('utf-8'))  # Cambiado a .encode
